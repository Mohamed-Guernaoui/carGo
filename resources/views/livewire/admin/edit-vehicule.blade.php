<div class="flex h-full w-full flex-1 flex-col gap-6 p-4">
    ...
    <div class="flex items-center justify-between gap-4 flex-wrap">
        <div>
            <a href="{{ route('admin.vehicules.show', $vehicule->id) }}" class="inline-flex items-center text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 font-medium text-sm">
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                Back to Vehicle Details
            </a>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white mt-2">
                Edit {{ $vehicule->marque }} {{ $vehicule->modele }} ({{ $vehicule->annee }})
            </h1>
            <p class="text-gray-600 dark:text-gray-400">Update the details for your rental vehicle.</p>
        </div>
    </div>

    <div class="rounded-xl border border-neutral-200 bg-white p-6 dark:border-neutral-700 dark:bg-neutral-800">
        <form wire:submit.prevent="updateVehicule" class="space-y-6">
            <div>
            ...
            <div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="new_image" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Upload New Vehicle Image (Optional)</label>
                        <input wire:model="new_image" type="file" id="new_image" accept="image/*"
                        ...
                        @if ($new_image)
                            <div class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                New Image Preview:
                                <img src="{{ $new_image->temporaryUrl() }}" class="mt-1 max-h-40 rounded-lg object-cover">
                            </div>
                        @elseif ($vehicule->primary_image_url)
                            <div class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                Current Image:
                                <img src="{{ $vehicule->primary_image_url }}" class="mt-1 max-h-40 rounded-lg object-cover">
                            </div>
                        @endif
                    </div>
                </div>
